...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        """Handle GET requests, serving plan.json from the plan directory."""
        # Handle plan.json (with or without query strings)
        if self.path == "/plan.json" or self.path.startswith("/plan.json?"):
            # Serve plan.json from the plan directory (not .pv/)
            try:
                ...
                self.send_error(404, "plan.json not found")
            return
        # Default handling for other files (dashboard.html, etc.)
        super().do_GET()
...
def cmd_dashboard(plan: dict, args: argparse.Namespace) -> None:
    ...
    plan_path = args.file.resolve()
    plan_dir = plan_path.parent
    cache_dir = plan_dir / ".claude" / "plan-view"

    # Create .claude/plan-view/ directory if it doesn't exist
    cache_dir.mkdir(parents=True, exist_ok=True)

    # Create .gitignore to exclude all contents
    gitignore_path = cache_dir / ".gitignore"
    if not gitignore_path.exists():
        gitignore_path.write_text("# Auto-generated by pv - ignore all contents\n*\n")

    # Write dashboard HTML to .claude/plan-view/
    ...
    # Change to cache directory for serving static files
    os.chdir(cache_dir)

    # Set plan path for handler (absolute path to plan.json)
    DashboardHandler.plan_path = plan_path
